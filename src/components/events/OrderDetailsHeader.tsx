...
export const OrderDetailsHeader = ({ order }: OrderHeaderProps): JSX.Element => {
  ...
  return (
    <List sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', backgroundColor: '#ffffff' }}>
      ...
      <ListItem sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 'auto' }}>
        ...
        <Typography variant='body1'>{formatDateTime(order.dateCreated)}</Typography>
      </ListItem>
      <ListItem sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 'auto' }}>
        <Typography variant='caption'>Broadcast time</Typography>
        <Typography variant='body1'>{formatDateTime(eventDetails.dateStart)}</Typography>
      </ListItem>
    </List>
  );
};
